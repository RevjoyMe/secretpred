...
contract PredictionMarket is IPredictionMarket {
    ...
    // Events are defined in interface
    ...
    modifier marketActive(uint256 marketId) {
        require(markets[marketId].state == uint256(MarketState.Active), "Market not active");
    ...
    function createMarket(
        ...
        markets[marketId] = Market({
            ...
            endTime: endTime,
            resolutionTime: 0,
            state: uint256(MarketState.Active),
            outcome: false,
            oracle: oracle,
            totalPool: 0,
            yesPool: 0,
    ...
    function resolveMarket(
        ...
        Market storage market = markets[marketId];
        require(market.state == uint256(MarketState.Active), "Market not active");
        require(block.timestamp >= market.endTime, "Market still active");
        
        market.state = uint256(MarketState.Resolved);
    ...
    function claimPayout(uint256 marketId) external validMarket(marketId) {
        Market storage market = markets[marketId];
        require(market.state == uint256(MarketState.Resolved), "Market not resolved");
